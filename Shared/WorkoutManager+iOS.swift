...
extension WorkoutManager {
    func fetchTodaysWorkouts(workoutType: HKWorkoutActivityType) async -> [HKWorkout] {
        let samples = try? await withCheckedThrowingContinuation { (continuation: CheckedContinuation<[HKSample], Error>) in
            ...
            let walkingPredicate = HKQuery.predicateForWorkouts(with: .hiking)
            let compoundPredicate = NSCompoundPredicate(andPredicateWithSubpredicates: [todayPredicate, walkingPredicate])
