...
class WorkoutManager: NSObject, ObservableObject{
    ...
    func getUserAge(){
        do{
            ...
            print("there's an error in calculating the age")
        }
    }
    ...
    // Pause the timer
    func pauseTimer() {
        guard timer != nil else { return } // Timer must be running
        pausedTime = remainingTime // Save the remaining time when paused
        stopTimer()
        isPaused = true
    }
    ...
    func resumeTimer() {
        guard isPaused else { return } // Can only resume if the timer is paused
